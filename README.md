...
- **Python** v.3.7.9 or higher (click [here](https://www.python.org/downloads/) to download).
...
A single and common method `display()` was designed to manage the entire system menus navigation. It is stored in `utils.py` [here](system/utils.py).
...
- If the user choice is to 'go back to main menu', execute a recurrsive call of the `display()` method with the correct main menu dictionary as argument.

- If the user choice is to logout, call `logout()` and execute a recurrsive call of the `display()` method with the main register/login menu dictionary.
...
For specific menus, the menu display and user input combined with input validation are inserted whithin the 'corresponding method' of the previous menu dictionary. Once the task is completed, the method just has to call `utils.display(`[following dictionary]`)` just like the other methods.
