...
| select | factory | <li><b>Admin, GP</b> <li> Generating an instance of an appointment to later update attributes based on user input | - | - |
...
| select | factory | <li><b>Admin</b> <li> Generating an instance of a GP to later update attributes based on user input | <li>gp_id | <li> GP instance <li> DF incl. indexing of all of a GP's attributes (except password) <li> DF of GP departments (for admin reference) <li> DF of GP specialisations (for admin reference) |
...
| select_upcoming_timeoff | static | <li><b>Admin</b> <li> Viewing a GP's upcoming timeoff (only in whole days for now) | <li>gp_id | <li>DF of a GP's upcoming time off {booking_id, booking_start_time, booking_status, booking_status_change_time} |
| check_timeoff_conflict | static | <li><b>Admin, GP</b> <li> Checking proposed GP timeoff doesn't conflict with any appointments | <li>gp_id <li>date_start <li>date_end | <li>DF of conflicting appointments {booking_id, booking_start_time, booking_status, booking_status_change_time, patient name (patient_first_name + patient_last_name)} <br><i>NB: in user flow, to 'check' for conflicts count rows in DF |
| insert_timeoff | static | <li><b>Admin, GP</b> <li> Inserting GP time off (only in whole days for now) | <li>type = day/week/custom <li>gp_id <li>reason = time off/sick leave <li>[time parameters] | - |
| delete_upcoming_timeoff | static | <li><b>Admin, GP</b> <li> Deleting a GP's upcoming time off (e.g if no longer sick, holiday cancelled) (only in whole days for now) | <li>type = all/day/week/custom <li>gp_id <li>[time parameters] | - |
<br>
